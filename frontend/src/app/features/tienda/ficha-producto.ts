  ...
  <div class="page">
    ...
    <div class="ficha">
      <div class="card galeria">
        <div class="big"><i class="fa-solid {{ p.icono }}"></i></div>
        <div class="thumbs"><span><i class="fa-solid {{ p.icono }}"></i></span><span><i class="fa-solid fa-box"></i></span><span><i class="fa-solid fa-tag"></i></span></div>
      </div>
      ...
      <div class="card detalle">
        ...
        <div class="d-feats">
          <span><i class="fa-solid fa-truck"></i> Delivery en Huacho</span><span><i class="fa-solid fa-file-invoice"></i> Boleta / Factura</span><span><i class="fa-solid fa-check"></i> Producto original</span>
        </div>
      </div>
    </div>
    } @else {
    ...
    @if (toast) { <div class="toast"><i class="fa-solid fa-check"></i> {{ toast }}</div> }
  </div>
  `,
  styles: [`
    .ficha { display:grid; grid-template-columns:0.9fr 1.1fr; gap:1.5rem; align-items:start; }
    .galeria .big { font-size:6rem; text-align:center; background:var(--glass-bg-strong); color:var(--accent-blue); border-radius:12px; padding:2.5rem 0; }
    .thumbs { display:flex; gap:.6rem; margin-top:.75rem; }
    .thumbs span { flex:1; text-align:center; font-size:1.4rem; color:var(--accent-blue); background:var(--glass-bg-strong); border:1px solid var(--glass-border); border-radius:8px; padding:.5rem 0; cursor:pointer; }
    .d-lab { color:var(--accent-blue); font-weight:700; font-size:.8rem; text-transform:uppercase; }
    .detalle h1 { font-size:1.6rem; margin:.25rem 0 .75rem; color:var(--text-primary); }
    .d-meta { display:flex; gap:1.5rem; margin:1rem 0; padding:1rem 0; border-top:1px solid var(--glass-border); border-bottom:1px solid var(--glass-border); }
    .d-meta div { display:flex; flex-direction:column; }
    .d-price { display:flex; align-items:center; gap:1rem; margin:.5rem 0; }
    .qty { display:flex; align-items:center; border:1px solid var(--glass-border-strong); border-radius:8px; overflow:hidden; }
    .qty button { border:none; background:var(--glass-bg-strong); color:var(--text-primary); width:38px; height:42px; font-size:1.2rem; cursor:pointer; }
    .qty input { width:54px; text-align:center; border:none; box-shadow:none; }
    .qty input:focus { box-shadow:none; }
    .d-feats { display:flex; gap:1.25rem; margin-top:1.25rem; font-size:.82rem; color:var(--text-secondary); flex-wrap:wrap; }
